...
import org.bootcamp.transactionorchestrator.kafka.event.PurchaseRejectedEvent;
...
@Slf4j
public class PurchaseRejectedConsumer {
    private final IdempotencyLogRepository idempotencyLogRepository;

    @KafkaListener(
        topics = "${topics.bank-transaction-purchase-rejected}",
        groupId = "${kafka.consumer.group-id}"
    )
    public void listen(ConsumerRecord<String, String> consumerRecord,
                       @Header(Constants.IDEMPOTENCY_KEY_HEADER) String idempotencyKey,
                       Acknowledgment ack) {
        log.info("Received purchase rejected event. idempotencyKey={}, offset={}",
            idempotencyKey, consumerRecord.offset());

        processPurchaseRejected(consumerRecord.value(), idempotencyKey)
            .doOnSuccess(unused -> ack.acknowledge())
            .doOnError(error -> log.error(
                "Error processing purchase rejected event. idempotencyKey={}, offset={}",
                idempotencyKey, consumerRecord.offset(), error))
            .subscribe();
    }

    private Mono<Void> processPurchaseRejected(String payload, String idempotencyKey) {
    ...
    private Mono<Void> processPendingOperation(String payload, IdempotencyLogDocument idempotencyLog) {
        ...
        PurchaseRejectedEvent event = IdempotencyUtils.deserializeResponse(payload, PurchaseRejectedEvent.class);
    ...
    private String buildFailedResponseBody(IdempotencyLogDocument idempotencyLog, PurchaseRejectedEvent event) {
